...
func TestLookupAPIKeyUpstreamModel(t *testing.T) {
		...
		ClaudeKey: []internalconfig.ClaudeKey{
			{
				...
				Models: []internalconfig.ClaudeModel{
					...
					{Name: "claude-opus-4(low)", Alias: "co4"},
				},
			},
		},
	}
	...
	tests := []struct {
		...
		{"config suffix priority", "a1", "co4(high)", "claude-opus-4(low)"},
		{"config suffix no user suffix", "a1", "co4", "claude-opus-4(low)"},
...
func TestAPIKeyModelAlias_ConfigHotReload(t *testing.T) {
	...
	mgr.SetConfig(&internalconfig.Config{
		ClaudeKey: []internalconfig.ClaudeKey{
			{
				APIKey: "k",
				Models: []internalconfig.ClaudeModel{{Name: "claude-opus-4", Alias: "cs4"}},
			},
		},
	})

	// New alias should take effect
	if resolved := mgr.lookupAPIKeyUpstreamModel("a1", "cs4"); resolved != "claude-opus-4" {
		t.Fatalf("after reload: got %q, want %q", resolved, "claude-opus-4")
	}
}
