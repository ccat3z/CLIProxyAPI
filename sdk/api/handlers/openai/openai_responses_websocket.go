...
func shouldReplaceWebsocketTranscript(rawJSON []byte, nextInput gjson.Result, allowCompactionReplayBypass bool) bool {
	...
	for _, item := range nextInput.Array() {
		...
		case "compaction", "compaction_summary":
			// Only trigger transcript replacement when the downstream supports
			// compaction replay. Otherwise, fall through to the merge path
			// where compaction items are stripped before merging.
