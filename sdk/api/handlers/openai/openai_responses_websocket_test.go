...
func TestShouldReplaceWebsocketTranscriptCodexLocalCompactionSemantics(t *testing.T) {
	...
	if !shouldReplaceWebsocketTranscript([]byte(`{"type":"response.create"}`), compactedInput, false) {
	...
	for _, request := range []string{
		...
		if shouldReplaceWebsocketTranscript([]byte(request), compactedInput, false) {
			t.Fatalf("request carrying previous_response_id must not use the local compaction rule: %s", request)
		}
	}
	if shouldReplaceWebsocketTranscript([]byte(`{"type":"response.append"}`), compactedInput, false) {
	...
	if shouldReplaceWebsocketTranscript([]byte(`{"type":"response.create"}`), ordinaryInput, false) {
...
func TestCodexLocalCompactionSummaryRejectsOrdinaryHistoryItems(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			if got := shouldReplaceWebsocketTranscript([]byte(`{"type":"response.create"}`), input, false); got != test.wantReplace {
