...
| `sdk/cliproxy/auth/conductor.go` | The redis-backed Home auth dispatch is inert: `Manager.HomeEnabled()` now reports `false`, `currentHomeDispatcher` returns `nil`, and the `publishErrorEvent` call was removed (the error-event publisher was deleted). The legacy home-mode retry/dispatch code paths remain compiled but are unreachable. |
